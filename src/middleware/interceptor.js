...
module.exports = function () {
	return async function (ctx, next) {
		// 解析用户信息
		const isLog = !logWhite.includes(ctx.url);
		if (isLog) {
			console.info(`${ctx.method} ${ctx.url} - 请求开始 --------------------`);
		}
		let log = {
			resourceUrl:'', // 请求url
		...
		try {
			...
			logsUtil.error(ctx, err, Date.now());
			ctx.log.responseMessage = err.message;
			ctx.body = statusCode.ERROR_AOTU(err.code || 500, err.stack || err.message);
		}
		log = ctx.log;
		...
		if(ctx.body) {
			...
			log.resourceUrl = ctx.request.url;
